...
if (isset($_POST['submit'])) {
  ...
  if ($email != "" && $password != "") {
    try {
      $query = "SELECT `id`, `name`, `email`, `mobile`, `password`, `create_date` FROM tbladmin WHERE email=:email LIMIT 1";
      $stmt = $dbh->prepare($query);
      $stmt->bindParam('email', $email, PDO::PARAM_STR);
      $stmt->execute();
      $count = $stmt->rowCount();
      $row = $stmt->fetch(PDO::FETCH_ASSOC);
      if ($count == 1 && !empty($row) && matchPasswords($password, $row['password'])) {
